...
func billingReportByCompetition(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitonID string) (*BillingReport, error) {
	...
	if err := adminDB.SelectContext(
		ctx,
		&vhs,
		"SELECT player_id, created_at AS min_created_at FROM latest_visit_history WHERE tenant_id = ? AND competition_id = ?",
		tenantID,
		comp.ID,
	...
	if err := tenantDB.SelectContext(
		ctx,
		&scoredPlayerIDs,
		"SELECT DISTINCT(player_id) FROM new_player_score WHERE tenant_id = ? AND competition_id = ?",
		tenantID, comp.ID,
	); err != nil && err != sql.ErrNoRows {
...
func competitionScoreHandler(c echo.Context) error {
	...
	if _, err = tx.ExecContext(
		ctx,
		"INSERT INTO new_player_score(id, tenant_id, player_id, competition_id, score, created_at, updated_at) VALUES"+placeHolders.String(),
		args...,
	); err != nil {
		return err
	}
...
func playerHandler(c echo.Context) error {
	...
	if err := tenantDB.GetContext(
		...
		"SELECT * FROM new_player_score WHERE tenant_id = ? AND player_id = ?",
		v.tenantID,
		p.ID,
	); err != nil {
...
func competitionRankingHandler(c echo.Context) error {
	...
	if _, err := adminDB.ExecContext(
		...
		return fmt.Errorf(
			"error Insert latest_visit_history: playerID=%s, tenantID=%d, competitionID=%s, createdAt=%d, updatedAt=%d, %w",
			v.playerID, tenant.ID, competitionID, now, now, err,
		)
	}
